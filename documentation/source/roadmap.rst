...
Each folder, e.g *\\earthquakes\\5*, contains ten(10) accelerograms  with gradual intensity(0.1 - 1.0 g).
The orchestrator reads each accelerogram. Afterward, it sends the accelerogram as an argument to the bridge submodel.
Finally, a selected irregularity is applied for the rails. This is included in the file *dev\\PyCoSimulation\\src\\Parameters.txt*. The *orchestrator* reads the irregularity and sends this information to the vehicle subsystem.


orchestrator
------------
The *orchestrator* class contains all the necessary parameters for the cosimulation analysis of the coupled VBI problem. The *orchestrator* manages the solution process. In the current implementation, a *Gauss-Seidel* communication scheme is adopted, i.e., that the two subsystems are solved sequentially. This class organizes the information at the interface of the coupled problem. A "Force-Displacement" coupling method is implemented. Specifically, the *orchestrator* receives from the bride subsystem the state of the rails(displacements, velocities) and sends this information to the vehicle subsystem. Afterward, the *orchestrator* receives from the vehicle subsystem the contanct forces (position, force vector) and sends this information to the bridge subsystem.  
...
The function *multiple analysis* sets and executes the analyses. It is called from *main*, constructs and solves each cosimulation analysis.
...
Also basic data of the cosimulation analysis are defined in this script 
...
The function *solution* runs the cosimulation analysis. Sends and receives the input/output of each subsystem, checks convergence conditions at the interfaces, and restores or updates (convergence) the systems states. Furthermore, it saves to specific structures selected results for post process.
...
The function *update*  updates the subsystems state when the process is converged
...
The function *restore* resets the current trial structure state (displacements, velocity, internal parameters) according to the previously converged state. 
The function *update* sets the converged state equal to the current trial.
